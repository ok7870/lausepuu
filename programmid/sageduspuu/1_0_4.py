...
tekst=open("../../tekstid/lihtne_tekst2.txt",encoding="utf-8").read().strip().split(".")
...
def printer( oks, rekusioon=1, filter=0, taane=0, seerekusioon=0):
    ...
    for sona in oks["oksad"]:
        if filter <= oks["oksad"][sona]["korrad"]:
            ...
            printer(oks["oksad"][sona], rekusioon, filter, taane+4, seerekusioon)



    
printer(puu, int(input("luse pikkus: ")), int(input("min kordusde arv: "))) #lause pikkus; sügavus