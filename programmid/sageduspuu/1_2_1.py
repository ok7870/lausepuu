...
if inpt == "text":
    ...
    for tuup in tuupid:
        ...
        if tuup in ["text", "lemma", "upos", "xpos", "feats"]:
            ...
            for failinimi in failinimed:
                ...
                for lause in range(len(stanzatudfail.sentences)):
                    ...
                    for sona in range(len(stanzatudfail.sentences[lause].words)):
                        if stanzatudfail.sentences[lause].words[sona].__getattribute__(tuup) in p_oks["oksad"]:
                            p_oks["oksad"][stanzatudfail.sentences[lause].words[sona].__getattribute__(tuup)]["korrad"]+=1
                        else:
                            p_oks["oksad"][stanzatudfail.sentences[lause].words[sona].__getattribute__(tuup)]={}
                            p_oks["oksad"][stanzatudfail.sentences[lause].words[sona].__getattribute__(tuup)]["korrad"]=1
                            p_oks["oksad"][stanzatudfail.sentences[lause].words[sona].__getattribute__(tuup)]["oksad"]={}
                        p_oks=p_oks["oksad"][stanzatudfail.sentences[lause].words[sona].__getattribute__(tuup)]
        else:
            print()
